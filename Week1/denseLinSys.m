% This shows that the time required to solve a dense linear system varies 
% with the order of a matrix. The linear system being solved is
% Sx = b, where S is the output of the randMat function on a size and
...
for i=1:length(n)
    % S is the output of the randMat function called on the ith size in
    % the list n.
    S = randMat(n(i));
